...
def main(_run, _config, _log):
    ...
    log_loss = {'total_loss': 0, 'query_loss': 0, 'align_loss': 0, 'qry_loss': 0, 'distr_loss': 0, 'class_loss': 0, 'thresh_loss': 0}
    ...
    for sub_epoch in range(n_sub_epochs):
        ...
        for _, sample in enumerate(train_loader):
            ...
            query_pred , align_loss , qry_loss , distr_loss , class_loss = model(support_images, support_fg_mask, query_images, query_labels,
                                           opt=optimizer, train=True)
            ...
            loss = query_loss + align_loss + qry_loss + distr_loss + class_loss #权重因子参考论文或根据实际情况修改
            ...
            qry_loss = qry_loss.detach().data.cpu().numpy()
            distr_loss = distr_loss.detach().data.cpu().numpy()
            class_loss = class_loss.detach().data.cpu().numpy()
            ...
            _run.log_scalar('qry_loss', qry_loss)
            _run.log_scalar('distr_loss', distr_loss)
            _run.log_scalar('class_loss', class_loss)

            log_loss['total_loss'] += loss.item()
            log_loss['query_loss'] += query_loss
            log_loss['align_loss'] += align_loss
            log_loss['qry_loss'] += qry_loss
            log_loss['distr_loss'] += distr_loss
            log_loss['class_loss'] += class_loss
            ...
            if (i_iter + 1) % _config['print_interval'] == 0:
                ...
                qry_loss = log_loss['qry_loss'] / _config['print_interval']
                distr_loss = log_loss['distr_loss'] / _config['print_interval']
                class_loss = log_loss['class_loss'] / _config['print_interval']

                log_loss['total_loss'] = 0
                log_loss['query_loss'] = 0
                log_loss['align_loss'] = 0
                log_loss['qry_loss'] = 0
                log_loss['distr_loss'] = 0
                log_loss['class_loss'] = 0

                _log.info(f'step {i_iter + 1}: total_loss: {total_loss}, query_loss: {query_loss}, '
                          f'align_loss: {align_loss}, '
                          f'qry_loss: {qry_loss}, distr_loss: {distr_loss}, class_loss: {class_loss},')
                # f' align_loss: {align_loss}')
